...
for (trn in trn.vector){
  ...
  bins=c(sm,
         ...
         nth(vec2,as.integer(l*0.995)), 1)##0.995
  ...
  # correlation <- read.csv(file = Mat.fname, header = T,check.names=FALSE)
  # rownames(correlation) <- colnames(correlation)
  #correlation <- correlation[as.character( newCluster$X),as.character(newCluster$X)]
  # correlation <- data.matrix(correlation)
  ...
  source_python("./module3/auto-cluster2.py")
  outidx <- get_GroupID(correlation, numC)
  
  # write.csv(rev(rownames(p$carpet)), file=file.path(results.path, "clustering", paste0("Newcluster_trn",trn,".csv")), row.names=TRUE)
  write.csv(outidx, file=file.path(results.path, "clustering", paste0("Newcluster_trn",trn,"_",numC,".csv")), row.names=TRUE)
...
message("\n~~~~~Rscript finished analyzing co-occurrence and clustering labelling")
